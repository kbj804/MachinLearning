...
def save_word(str_top_word, count_word, url_id,img_num,child_num):
    try:
        with connection.cursor() as curs:
            curs.execute(save_wd, (count_word, str_top_word, img_num, child_num, url_id))
            print("Insert Complete : ")
    except Exception as ex:
        print("Fail to save the Top_20 word ERROR : ", ex)


def save_text(url_id, text):
    try:
        with connection.cursor() as curs:
            if curs.execute("select url_id from en_data where url_id=%s",url_id):
                curs.execute(update_txt, (text,url_id))
            else:
                curs.execute(insert_txt, (url_id, text))
    except Exception as ex:
        with connection.cursor() as curs:
            curs.execute(fail_lang,url_id)
            print("Fail To Insert EN_DATA, ERROR : ", ex)
...
def count_img_num(generate_url):
    ...
    img_num = BeautifulSoup(html, 'html.parser', from_encoding='utf-8')
    imgs = img_num.find_all('img')
    #print(imgs)
    return len(imgs)
    ...
    while 1:
        try:
            ...
            for origin_url in generated:
                # origin_url[0] : url_id
                # origin_url[1] : url
                most20_word, count, text, img_num, child_num = parse_word(origin_url[1]) # top 20 word 정보 str로 반환
                save_word(most20_word, count, origin_url[0],img_num,child_num)
                save_text(origin_url[0], text)
                #print(most20_word)
